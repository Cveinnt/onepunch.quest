...
<script>
	...
	// ------------------------------------------------------------------------
	let lastDay = ($state.set !== 1) ? training[$state.level][$state.set]['situps'].reduce((a, b) => a + b, 0) : 0;
	$: commingUp = training[$state.level][$state.set]['situps'].reduce((a, b) => a + b, 0);
	$: sidebarData = [$state.situpsLast, commingUp, $state.set];
